...
var BASE = process.env.BRPIX_BASE_URL || "https://brpixpayments.com.br";
...
/* Cria cobrança (cash-in). body: {amount, description, external_reference, expires_in?}
   Resposta real (HTTP 202) é FLAT: { txid, id, qr_code_text, qr_code_image, pix_copia_e_cola,
   expires_at, status, external_reference, poll_url }. Normalizamos aqui. */
async function createCashIn(body) {
  ...
  var pix = data.pix || {};
  return {
    ok: res.ok, status: res.status, raw: data,
    txid: data.txid || data.id || null,
    qr_code: data.qr_code_text || data.pix_copia_e_cola || pix.qr_code || null,
    qr_code_image: data.qr_code_image || pix.qr_code_image || null,
    expires_at: data.expires_at || pix.expires_at || null,
    error: data.error || data.error_code || null
  };
}
